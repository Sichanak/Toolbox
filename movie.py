...
def reply_message(title):
    ...
    formated_fields = []
    formated_cover = '< a href="{0}">&#8204;</a>'.format(title['full-size cover url'])
    title_line = '<b>{0} ({1}) | {2}</b>\n\n'.format(title[bold_fields[0]], title['year'], title['kind'])
    for field in bold_fields[1:]:
        line = '<b>{0}:</b> {1}'.format(field.title(), title[field])
...
class Alert():
    ...
    def _get_movie_release_date(self, user_id, user_name, title_id, title_name):
        ...
        if release_dates:
            usa_release_date = [i['date'] for i in release_dates if i['country'] == 'USA\n' and not i.get('notes')]
            if usa_release_date and match(date_regex, usa_release_date[0]):
                release_date = datetime.strptime(usa_release_date[0], '%d %B %Y')
                if release_date > datetime.now():
                    db_values = (user_id, user_name, title_id, title_name, None, release_date)
                ...
                if ep_release_date and match(date_regex, ep_release_date):
                    ...
                    if release_date < datetime.now():
                        if ep_no == len(latest_episodes):
                            message = 'Season {0} finale aired {1}'.format(current_season, ep_release_date)
                            return message
                    else:
                        title_episode = ep_data.getID()
                        db_values = (user_id, user_name, title_id, title_name, title_episode, release_date)
                        message = self.db_api.insert(db_values)
                        return message
                else:
    ...
    def _update_episode(self, user_id, title_id, current_episode_data):
        ...
        if next_episode_id:
            ...
            if next_release_date and match(date_regex, next_release_date):
                ...
                release_date = next_week_date.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    def enable(self, user_id, user_name, title_id):
        ...
        if seasons:
            message = self._get_episode_release_date(user_id, user_name, title_id, title_name)
        else:
            message = self._get_movie_release_date(user_id, user_name, title_id, title_name)
        return message
